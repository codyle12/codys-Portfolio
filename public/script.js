(function () {
  ...
  document.addEventListener("pointerdown", () => {
    initAudio();
    if (!audioCtx.preloaded) {
      audioCtx.preloaded = true;
      [clickEl, darkOnEl, darkOffEl, failEl].forEach(async (el) => {
        if (!el || !el.src) return;
        try {
          const res = await fetch(el.src);
          const data = await res.arrayBuffer();
          sfxBuffers[el.id] = await audioCtx.decodeAudioData(data);
        } catch (e) {}
      });
    }
  ...
  function playEl(el) {
    ...
    if (sfxBuffers[el.id]) {
      ...
      source.buffer = sfxBuffers[el.id];
  ...
  const SFX = {
    click() { playEl(clickEl); },
    darkToggle(on) { on ? playEl(darkOnEl) : playEl(darkOffEl); },
    fail() { playEl(failEl); }
  };
...
document.querySelectorAll('.nav-link').forEach(link => {
    link.addEventListener('click', () => {
      if (window.innerWidth <= 768) {
        setTimeout(() => {
          const openWindows = Array.from(document.querySelectorAll('.app-window.open'));
          openWindows.sort((a, b) => parseInt(b.style.zIndex || 0) - parseInt(a.style.zIndex || 0));
          
          for (let i = 1; i < openWindows.length; i++) {
            const win = openWindows[i];
            const closeBtn = win.querySelector('.controls button') || win.querySelector('.controls');
            if (closeBtn) {
              closeBtn.click();
            } else {
